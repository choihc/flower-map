...
`apps/web/src/lib/types.ts` 파일에 아래 내용을 기존 타입들 뒤에 추가:
...
**Files:**
- Create: `apps/web/src/lib/data/spotPhotos.ts`
...
```bash
git add apps/web/src/lib/data/spotPhotos.ts
...
**Files:**
- Modify: `apps/web/src/features/import/importSchema.ts`
- Modify: `apps/web/src/features/import/importSchema.test.ts`
...
```bash
cd apps/web && npm test -- importSchema
```
...
```bash
git add apps/web/src/features/import/importSchema.ts apps/web/src/features/import/importSchema.test.ts
...
**Files:**
- Modify: `apps/web/src/features/import/planImportWrite.ts`
...
```bash
cd apps/web && npm test -- planImportWrite
```
...
```bash
git add apps/web/src/features/import/planImportWrite.ts
...
**Files:**
- Modify: `apps/web/src/features/import/actions.ts`
...
```bash
cd apps/web && npx tsc --noEmit 2>&1 | grep actions
```
...
```bash
git add apps/web/src/features/import/actions.ts
...
**Files:**
- Create: `apps/web/src/features/spots/photoActions.ts`
...
```bash
git add apps/web/src/features/spots/photoActions.ts
...
주의: `Button` 컴포넌트가 `variant`, `size` prop을 지원하지 않을 수 있음. `apps/web/src/components/ui/button.tsx`를 확인하고 맞게 조정.

- [ ] **Step 2: TypeScript 오류 확인**

```bash
cd apps/web && npx tsc --noEmit 2>&1 | grep SpotForm
```
...
```bash
git add apps/web/src/features/spots/SpotForm.tsx
...
```bash
cd apps/web && npx tsc --noEmit 2>&1 | grep "spots/\[id\]"
```
...
```bash
git add apps/web/app/(dashboard)/spots/[id]/page.tsx
