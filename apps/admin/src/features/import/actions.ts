...
export async function saveImportPayloadAction(payload: string): Promise<ValidationSummary> {
  ...
  try {
    ...
    const incomingSlugs = ('spot' in parsed.data ? [parsed.data.spot] : parsed.data.spots).map((spot) => spot.slug);
    ...
    for (const spot of plan.toCreate) {
      await createSpot(writeClient, spot);
    }
...
async function resolveFlowerId(
  ...
  if ('spot' in payload) {
    ...
    if (flower == null) {
      throw new Error(`등록된 꽃이 없습니다: ${payload.flower_slug}`);
    }
...
async function listSpotIdentitiesBySlugs(
  ...
  if (slugs.length === 0) {
    return [] as Array<Pick<SpotRow, 'id' | 'slug'>>;
  }
